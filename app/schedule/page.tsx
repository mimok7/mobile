    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="sticky top-0 z-30 bg-white border-b shadow-sm px-4 py-3">
          ...
          {(['day', 'week', 'month'] as ViewMode[]).map(mode => (
            <button
              ...
              className={`flex-1 py-1.5 text-xs rounded-lg font-medium transition-colors ${
                viewMode === mode ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
            >
        ...
        <div className="flex gap-1 overflow-x-auto pb-1 -mx-1 px-1">
          ...
          {Object.entries(serviceConfig).map(([key, { name }]) => (
            <FilterBtn key={key} label={name} active={typeFilter === key} onClick={() => setTypeFilter(key)} />
          ))}
        </div>
      </div>
